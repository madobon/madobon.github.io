...
- After every push to `madobon.github.io`, check the GitHub Actions / CI result for the pushed commit and confirm that all required jobs succeeded before reporting completion.
- Prefer a watch-style workflow for CI confirmation: identify the run with `gh run list --commit <sha>`, then wait for completion with `gh run watch <run-id>`.
